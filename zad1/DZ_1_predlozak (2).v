...
Lemma ALU_One (z : nat) (x y : list B) :
  length x = z /\ length y = z /\ z <> 0 -> ALU x y I I I I I I = repeat O (pred z) ++ [I].
Proof.
  intros [H_z [H_z_y n0]]. simpl. rewrite H_z, H_z_y.
  assert (nl : forall (n:nat), NotL (repeat O n) = repeat I n).
    -induction n as [| n' IH]; simpl; [reflexivity | rewrite IH; reflexivity].
    -rewrite nl. unfold AddL. assert (a : forall (b:B) (n:nat), repeat b n ++ [b] = b :: repeat b n).
    --intros b n. induction n as [| n' IH]; simpl; [reflexivity | rewrite IH; reflexivity].
    --assert (rev : forall (b:B) (n : nat), rev (repeat b n) = repeat b n).
    intros b n.
    ---induction n as [| n' IH]; simpl; [reflexivity | rewrite IH, a; reflexivity].
    ---rewrite rev. destruct z as [| z'].
    contradiction n0. reflexivity.
    simpl. assert (addlr : forall (n : nat), AddLr (repeat I n) (repeat I n) I = repeat I n).
    ----induction n as [| n' IH'']; simpl; [reflexivity | rewrite IH''; reflexivity].
    ----rewrite addlr. rewrite rev.
    assert (endd : forall (n:nat), NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
    -----induction n as [| n' IH''']; simpl; [reflexivity | rewrite IH'''; reflexivity].
    -----apply endd.
Qed.
